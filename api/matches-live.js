...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      'https://esport.is/api/matches/live?game=valorant',
    ...
    const transformed = transformLiveMatches(raw);
...
function transformLiveMatches(raw) {
  const matches = (raw.data || raw.matches || raw || []).map(m => ({
    ...
    map: m.current_map || m.map || '',
    mapScoreA: m.map_score_a ?? m.map_wins_a ?? 0,
    mapScoreB: m.map_score_b ?? m.map_wins_b ?? 0,
